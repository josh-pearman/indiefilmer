...
async function main(): Promise<void> {
  ...
  const dbChoice = await select({
    message: "Database",
    options: [
      { value: "sqlite", label: "SQLite (default, no setup required)" },
      { value: "postgresql", label: "PostgreSQL (requires a running Postgres instance)" }
    ]
  });
  if (dbChoice === undefined || typeof dbChoice !== "string") {
    cancel("Setup cancelled.");
    process.exit(0);
  }

  let databaseUrl = 'file:../data/db.sqlite';
  if (dbChoice === "postgresql") {
    const pgUrl = await text({
      message: "PostgreSQL connection string",
      placeholder: "postgresql://user:password@localhost:5432/indiefilmer",
      validate: (v) => {
        const s = v != null ? String(v).trim() : "";
        if (!s) return "Required";
        if (!s.startsWith("postgresql://") && !s.startsWith("postgres://")) return "Must start with postgresql:// or postgres://";
        return undefined;
      }
    });
    if (pgUrl === undefined) {
...
Your team can then access the app at https://your-domain.com`,
  ...
  // Swap Prisma provider if needed and set up database
  note("Setting up database…", "Step 2");
  if (dbChoice === "postgresql") {
    run("npx", ["tsx", "scripts/prisma-provider.ts"]);
    run("npx", ["prisma", "generate"]);
    if (!run("npx", ["prisma", "db", "push"])) {
      cancel("Database setup failed. Check your PostgreSQL connection string.");
      process.exit(1);
    }
  } else {
    if (!run("npx", ["prisma", "migrate", "deploy"])) {
      cancel("Database migration failed.");
      process.exit(1);
    }
  }
  ...
  const userCount = await checkDb.user.count();
